...
**Официальный результат 17 сентября 2026: 20/20, медиана 0,701 мс —
ускорение 3,61× относительно исходного baseline.**
На проверке 17 сентября, 16:09 МСК: **3-е место из 6**.
...
[Новый полный запуск](https://www.fherma.io/kernels/polynomial-multiplication/specifications/negacyclic/runs/6aabe5b4e3bdd1a5530b9faf),
...
подтверждённый конкурсный коммит — `20f262efe701d58cbfb723991bd1b5e7e4044d35`.
...
Обмен использует постоянные pinned-буферы. Вход делится на четыре части:
CPU копирует следующую часть, пока DMA передаёт предыдущую. CUDA Graph
содержит вычислительные ядра. Выход также передаётся четырьмя частями;
готовность каждой части подтверждает собственное CUDA-событие, после чего
восемь CPU-потоков копируют её в обычный выходной std::vector. Следующая DMA
может идти одновременно с этим копированием. Выделение и заполнение вектора
перекрываются вычислением GPU; все операции и ожидания остаются в run.

Последний полный прогон: среднее 0,696 мс, медиана 0,701 мс, максимум 0,751 мс.
Короткий прогон того же коммита дал 0,637 мс; это отдельное измерение на трёх
случаях, которое не заменяет полный конкурсный результат.

Также реализован экспериментальный `rns/solve.cu`: 57 малых простых,
нативная NTT и точная CRT-реконструкция через cuPQC BigInt1024. Он проверен
независимым эталоном и на GPU, но ещё не улучшил полный конкурсный score.
Математические границы и журнал: `research/RNS_PLAN_RU.md`, `EXPERIMENTS_RU.md`.
Backend выбирается параметром CMake `-DFHERMA_SOLVER=wide` или `rns`;
default текущего HEAD может соответствовать активному эксперименту.
...
```sh
cmake -B build -DCMAKE_BUILD_TYPE=Release -DFHERMA_SOLVER=wide
cmake --build build -j

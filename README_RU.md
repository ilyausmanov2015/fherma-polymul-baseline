...
**Официальный результат 17 сентября 2026: 20/20, медиана 0,363 мс —
ускорение 6,98× относительно исходного baseline.**
На проверке 17 сентября, 23:44 МСК: **3-е место из 6**.
...
| Чередование четырёх pinned-буферов вывода | 4239f41 | 20/20 | 0,363 мс | 0,354–0,375 мс |

Два дополнительных прогревочных случая в каждом запуске также прошли,
но не входят в score. CUDA 12.8.61, cuPQC 0.6.0.

[Новый полный запуск](https://www.fherma.io/kernels/polynomial-multiplication/specifications/negacyclic/runs/6aac5047b615118421bfbb96),
...
В `main` включено чередование четырёх выходных буферов; профилирование
и остальные экспериментальные варианты выключены. Для воспроизведения конкурсного результата
используйте точный коммит
`4239f41c068034f1689ae0cd4f5393737827a68c`.

Последнее улучшение: 378,0135 → **362,920 мкс**, на 4,0% быстрее.
Отдельный benchmark нового варианта: 20/20, **364,925 мкс**.
GMP N32768: 9/9; CUDA CI и проверки CPU-копирования успешны.
Дополнительная pinned memory — 10,5 МиБ. Каждый run полностью записывает
новый результат в очередной буфер и возвращает свежий обычный std::vector.
Подробная диагностика: [передачи и временная шкала](research/TRANSFER_TIMELINE_RU.md).

Дополнительные полные benchmark: radix-16 — 379,5845 мкс,
компактный цикл fused NTT — 377,348 мкс, fused NTT с обычным H2D —
381,7705 мкс; все 20/20. Отличие компактного цикла на 0,18% пока
не доказывает стабильного выигрыша. Это эксперименты до чередования буферов.
Контроль восстановленного `main` (`879f98f`): 27/27 GMP, CUDA CI успешен,
GPU benchmark 20/20 — 375,0745 мкс. Он согласуется с лучшим конкурсным
результатом и показывает величину вариации между отдельными запусками.
Профиль рабочего конвейера: mapped prepare ~178 мкс, NTT/CRT ~64 мкс,
вывод с уведомлениями ~101–106 мкс; это диагностические интервалы с
добавленными событиями, а не отдельные слагаемые обычного score.
...
3. В run 16 CPU-потоков упаковывают вход четырьмя частями в mapped
   write-combined pinned memory. GPU читает каждую плитку из 32 коэффициентов
   один раз для всех 16 простых, формируя 64 канала остатков. Отдельного H2D
   нет; подготовка на GPU перекрывается с заполнением следующих частей на CPU.
4. Два прямых Harvey NTT выполняются совместно: отдельная
   bit-reversal перестановка и отдельное умножение на twist не требуются. В блоке 1024 значения три стадии
...
6. Выход передаётся восемью частями; 8 CPU-потоков читают каждую часть
   ...
   Между вызовами чередуются четыре собственных pinned-буфера: GPU не
   перезаписывает сразу буфер, который только что читали CPU-потоки.
...
Полный конкурсный прогон 4239f41: медиана 362,920 мкс, среднее 364,3478 мкс,
диапазон 354,171–374,724 мкс. Независимый benchmark 20/20 — 364,925 мкс.
...
4239f41 из начала документа; журнал содержит отдельные коммиты экспериментов.

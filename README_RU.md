...
**Официальный результат 17 сентября 2026: 20/20, медиана 0,952 мс —
ускорение 2,66× относительно исходного baseline.**
На проверке 17 сентября, 14:50 МСК: **4-е место из 6**.
...
[Новый полный запуск](https://www.fherma.io/kernels/polynomial-multiplication/specifications/negacyclic/runs/6aabd330e3bdd1a5530ad51e),
...
подтверждённый конкурсный коммит — `48d4d1bd1aff3eeb46f78abcdd11fb443a61f966`.
...
CPU-копирование входов двумя потоками входит в тайминг. Выход передаётся DMA
сразу в собственный std::vector; во время GPU-вычисления готовится следующий
вектор. Его выделение, заполнение и регистрация также входят в тайминг.
Перед возвратом результата регистрация снимается. В полном прогоне встречались
медленные случаи около 2 мс; среднее 1,279 мс при медиане 0,952 мс. Причина
разброса исследуется отдельным профилем CPU.

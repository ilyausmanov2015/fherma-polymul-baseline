...
**Официальный результат 17 сентября 2026: 20/20, медиана 0,597 мс —
ускорение 4,24× относительно исходного baseline.**
...
[Новый полный запуск](https://www.fherma.io/kernels/polynomial-multiplication/specifications/negacyclic/runs/6aabe7b2e3bdd1a5530bbb83),
...
подтверждённый конкурсный коммит — `e064b5297c97c34a4c31b8840b691c6623b171bc`.
...
5. cuPQC BigInt1024 накапливает взвешенные CRT-остатки. Целая fixed-point
...
6. Выход передаётся четырьмя частями. CPU читает каждую часть только после
   её CUDA-события; копирование может идти одновременно со следующей DMA.
...
Полный прогон e064b52: медиана 597,0275 мкс, среднее 599,765 мкс,
диапазон 576,038–632,840 мкс. Короткий прогон — 582,885 мкс, 3/3.
...
конкурсного результата используйте точный коммит e064b52 из начала документа.

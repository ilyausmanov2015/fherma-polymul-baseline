...
int main() {
    ...
    for(auto n:sizes) for(auto src:offsets) for(auto dst:offsets) {
        ...
        host_copy_bytes(got.data()+dst,in.data()+src,n);
        if(got!=expected) {
    ...
    std::printf("Host copy: 325 cases passed; AVX-512 available=%d\n",bool(__builtin_cpu_supports("avx512f")));
#else
    std::puts("Host copy: 325 cases passed; portable fallback");
#endif
}
